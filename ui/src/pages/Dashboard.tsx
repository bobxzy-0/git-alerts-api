...
export const Dashboard: React.FC = () => {
  ...
  return <div className="space-y-7">
    ...
    <section><div className="mb-3 flex items-center justify-between"><div><h2 className="text-lg font-semibold">Risk distribution</h2><p className="text-xs text-muted-foreground">按 Finding 风险等级统计</p></div><Link to="/findings" className="text-sm font-medium text-primary">View findings →</Link></div><div className="grid grid-cols-2 gap-3 md:grid-cols-5">{(['CRITICAL','HIGH','MEDIUM','LOW','INFO'] as const).map(level => <div key={level} className={`rounded-xl border p-4 ${severityStyle[level]}`}><p className="text-xs font-semibold tracking-wide">{level}</p><p className="mt-2 text-2xl font-bold">{data.severity_counts[level]}</p></div>)}</div></section>

    <div className="grid gap-6 xl:grid-cols-3">
      <section className="rounded-xl border bg-card p-5 shadow-sm xl:col-span-2"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-semibold">Scan Trend</h2><p className="text-xs text-muted-foreground">最近 14 天扫描次数</p></div><Link to="/scans?tab=history" className="text-sm text-primary">Task history →</Link></div><div className="flex h-52 items-end gap-1.5 border-b border-l px-2 pt-8">{data.scan_trend.map((item, index) => <div key={item.date} className="group relative flex h-full min-w-0 flex-1 items-end"><div className="relative w-full rounded-t bg-sky-500 transition-colors hover:bg-sky-400 dark:bg-sky-400 dark:hover:bg-sky-300" style={{height:`${item.count === 0 ? 2 : Math.max(10, item.count / maxTrend * 100)}%`}}><span className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-semibold text-foreground">{item.count}</span><div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-8 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-slate-900 px-2 py-1 text-xs text-white shadow-lg group-hover:block">{item.date} · {item.count} scans</div></div>{(index % 2 === 0 || index === data.scan_trend.length - 1) && <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] text-muted-foreground">{item.date.slice(5)}</span>}</div>)}</div><div className="h-6"/></section>

      <section className="rounded-xl border bg-card p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold">Source Health</h2><Link to="/source-health" className="text-sm text-primary">View all</Link></div><div className="space-y-3">{data.source_health.length === 0 ? <Empty text="No source health data"/> : data.source_health.map(item => <div key={item.id} className="rounded-lg border p-3"><div className="flex items-center justify-between"><strong className="capitalize">{item.source}</strong><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${healthStyle[item.status]}`}>{item.status}</span></div><div className="mt-3 grid grid-cols-3 gap-2 text-center"><Mini label="Results" value={item.result_count}/><Mini label="New" value={item.new_findings}/><Mini label="Limit" value={item.rate_limit_remaining ?? '—'}/></div>{item.error_message && <p className="mt-2 truncate text-xs text-destructive" title={item.error_message}>{item.error_message}</p>}</div>)}</div></section>
    </div>

    <div className="grid gap-6 lg:grid-cols-2">
      <section className="rounded-xl border bg-card shadow-sm"><div className="flex items-center justify-between border-b p-5"><h2 className="font-semibold">Recent Scans</h2><Link to="/scans?tab=history" className="text-sm text-primary">View all</Link></div>{data.recent_scans.length === 0 ? <Empty text="No scans yet"/> : <div className="divide-y">{data.recent_scans.map(scan => <Link key={scan.id} to={`/scans/${scan.id}`} className="flex items-center justify-between gap-4 p-4 hover:bg-muted/60"><div className="min-w-0"><p className="truncate text-sm font-medium">{scan.value}</p><p className="mt-1 text-xs text-muted-foreground">{scan.source} · {scan.trigger_type} · {new Date(scan.created_at).toLocaleString()}</p></div><span className="flex shrink-0 items-center gap-2 text-xs"><i className={`h-2 w-2 rounded-full ${statusDot[scan.execution_status] || 'bg-slate-400'}`}/>{scan.execution_status}</span></Link>)}</div>}</section>
...
const Metric = ({label,value,hint,accent=''}:{label:string;value:number;hint:string;accent?:string}) => <div className="rounded-xl border bg-card p-5 shadow-sm"><p className="text-sm text-muted-foreground">{label}</p><p className={`mt-2 text-3xl font-bold ${accent}`}>{value}</p><p className="mt-2 text-xs text-muted-foreground">{hint}</p></div>;
const Mini = ({label,value}:{label:string;value:string|number}) => <div><p className="text-sm font-semibold">{value}</p><p className="text-[10px] text-muted-foreground">{label}</p></div>;
