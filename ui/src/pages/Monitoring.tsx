...
function scheduleLabel(rule: MonitorRule) {
  ...
  if (rule.schedule_kind === 'DAILY') return `Daily at ${rule.schedule_time.slice(0, 5)}`;
  if (rule.schedule_kind === 'WEEKLY') return `${rule.schedule_weekdays.map(day => weekdayNames[day]).join(', ')} at ${rule.schedule_time.slice(0, 5)}`;
  return rule.interval_minutes < 60 ? `Every ${rule.interval_minutes} minutes` : `Every ${rule.interval_minutes / 60} hours`;
}
    ...
    {isLoading ? <div className="rounded-xl border p-8 text-center text-muted-foreground">Loading...</div>
      ...
      : <div className="overflow-x-auto rounded-xl border bg-card shadow-sm"><table className="w-full min-w-[1040px] text-sm">
        ...
        <tbody className="divide-y">{scheduledRules.map(rule => <tr key={rule.id} className="hover:bg-muted/30">
          ...
          <td className="max-w-xs px-4 py-4"><p className="truncate font-mono text-xs" title={rule.value}>{rule.value}</p></td><td className="px-4 py-4">{scheduleLabel(rule)}</td><td className="whitespace-nowrap px-4 py-4">{rule.enabled ? fmt(rule.next_run_at) : '—'}</td>
