...
function EditPlanModal({ rule, busy, onClose, onSave }: { rule: MonitorRule; busy: boolean; onClose: () => void; onSave: (data: Partial<MonitorRule>) => void }) {
  ...
  const availableTypes: ScanType[] = form.source === 'github' ? ['org_repos', 'search_repos', 'search_code', 'search_commits', 'search_issues', 'search_users'] : form.source === 'you' ? ['search_repos'] : ['org_repos', 'search_repos'];
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <form className="max-h-[90vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded-xl border bg-card p-6 shadow-xl" onSubmit={event => { event.preventDefault(); onSave({ name: form.name, value: form.value, source: form.source, scan_type: form.scan_type, schedule_kind: form.schedule_kind, interval_minutes: form.interval_minutes, schedule_time: form.schedule_time, schedule_weekdays: form.schedule_weekdays, cron_expression: form.cron_expression, timezone: form.timezone }); }}>
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm"><span className="mb-1 block font-medium">Source</span><select value={form.source} onChange={e => { const source=e.target.value as SourceType; setForm(current => ({...current, source, scan_type: source === 'you' ? 'search_repos' : 'org_repos'})); }} className="w-full rounded-md border bg-background px-3 py-2"><option value="github">GitHub</option><option value="gitlab">GitLab</option><option value="gitee">Gitee</option><option value="you">You.com Search</option></select></label><label className="text-sm"><span className="mb-1 block font-medium">Scan Type</span><select value={form.scan_type} onChange={e => set('scan_type', e.target.value as ScanType)} className="w-full rounded-md border bg-background px-3 py-2">{availableTypes.map(type => <option key={type} value={type}>{type}</option>)}</select></label></div>
