...
export type FindingReviewStatus = 'OPEN'|'CONFIRMED'|'FALSE_POSITIVE'|'IGNORED'|'RESOLVED';
export interface Finding { id:number; scan:number|null; last_scan:number|null; source:string; repository:string; type:string; value:string; secret_hash:string; fingerprint:string; description:string; file:string; line:number|null; email:string; commit_hash:string; commit_url:string|null; validated:boolean; review_status:FindingReviewStatus; lifecycle_status:'NEW'|'ACTIVE'|'ACKNOWLEDGED'|'RESOLVED'|'REOPENED'|'IGNORED'|'FALSE_POSITIVE'; severity:'CRITICAL'|'HIGH'|'MEDIUM'|'LOW'|'INFO'; risk_score:number; first_seen_at:string; last_seen_at:string; occurrence_count:number; created_at:string; updated_at:string; }
